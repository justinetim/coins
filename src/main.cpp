...
const string strMessageMagic = "CowCoin Signed Message:\n";
...
{
    // CowCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static darkgravitywave(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
    ...
    /// debug print
    // printf("Difficulty Retarget - DGW\n");
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	return darkgravitywave(pindexLast, pblock, BlocksTargetSpacing, PastBlocksMin, PastBlocksMax);
}
...
{
    // CowCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xd3, 0x3a, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CowCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CowCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CowCoinMiner(CWallet *pwallet)
{
    printf("CowCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CowCoin-miner");
    ...
    try { loop {
        ...
        printf("Running CowCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CowCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CowCoinMiner, pwallet));
}
