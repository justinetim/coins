...
// Copyright (c) 2011-2012 CowCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CowCoinTEST0\r");
            Send(hSocket, "WHO #CowCoinTEST0\r");
        } else {
            // randomly join #CowCoin00-#CowCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #CowCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CowCoin%02d\r", channel_number).c_str());
        }
